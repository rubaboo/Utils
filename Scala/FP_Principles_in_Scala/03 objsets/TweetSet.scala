...
abstract class TweetSet {
  ...
  def mostRetweeted: Tweet = {
    if(tail isEmpty) head
      ...
      val now = head
      val next  = tail.mostRetweeted

      if(now.retweets > next.retweets) now
      else next
    }
  }
  ...
  def descendingByRetweet: TweetList = {
    if(isEmpty) Nil
      ...
      val now = mostRetweeted
      new Cons(now, this.remove(now).descendingByRetweet)
    }
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    if(p(elem)) {
      (left union right).filter(p).union(acc.incl(elem))
    }
    else {
      (left union right).filter(p).union(acc)
    }
  }
